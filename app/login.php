            ...
            if(mysqli_num_rows($query)==1){
                if($fetch['login_status']=='verified'){
                    ...
                    if(mysqli_query($conn,$sql)){
                        ...
                        $json["phonenumber"] = $fetch['number'];
                        $json["gender"] = $fetch['gender'];
                        $json["bday"] = date( 'M. d, Y', strtotime($fetch['bday']));
                        $json["ProvName"] = $fetch['user_prov'];
                        $json["CityName"] = $fetch['user_city'];
                        $json["BrgyName"] = $fetch['user_brgy'];
                        $json["interest_count"] = interest_count($user_id);
                    }else{
                        $json["success"] = false;
